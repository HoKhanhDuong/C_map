                         ...
                         /* "p" if filename is a command for popen */
{
  ...
  is = talloc(struct inputstruct, 1); // cấp phát động cho is (line 5)
...
int get_line(IS is)// đọc từng dòng 
{
  int i, len;
  int f;
  char *tmp;
  ...
  if (fgets(is->text1, MAXLEN-1, is->f) == NULL) { // đặt nội dung của dòng trong text 1
    is->NF = -1;
    return -1;
  }
